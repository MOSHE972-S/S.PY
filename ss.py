...
try:
    ...
    # הסרנו את Fernet ועברנו ל-AES-GCM בלעדי
...
VERSION = "v18.0 Hebrew Edition (Secured)"
CHUNK_SIZE = 4 * 1024 * 1024  # 4MB
MAGIC_HEADER = b'IRON_V18'
...
class HebrewDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, title, message, mode="input", is_password=False):
        ...
        self.lbl = ctk.CTkLabel(self, text=message, font=("Segoe UI", 14), text_color=THEME["text"], wraplength=400, justify="right")
...
class CryptoEngine:
    ...
    def derive_kek(self, password: str, salt: bytes) -> bytes:
        # שודרג ל-n=262144 כדי להקשות משמעותית על כרטיסי מסך
        # מחזיר bytes נקיים (אין צורך ב-base64 יותר)
        return hashlib.scrypt(password.encode(), salt=salt, n=262144, r=8, p=1, dklen=32)
    ...
    def create_key_file(self, path, password):
        ...
        # הצפנת המפתח באמצעות AES-256-GCM במקום Fernet
        aes_kek = AESGCM(kek)
        nonce = os.urandom(12)
        encrypted_key_blob = aes_kek.encrypt(nonce, master_key, None)
        
        with open(path, "wb") as f:
            # מבנה הקובץ: מלח (16) + Nonce (12) + מפתח מוצפן ותג אימות (48)
            f.write(salt + nonce + encrypted_key_blob)
    ...
    def load_key_file(self, path, password):
        ...
        if len(data) < 76: raise ValueError("קובץ מפתח לא תקין או פגום")
        ...
        try:
            master_key = aes_kek.decrypt(nonce, enc_blob, None)
            self.secure_key_obj = SecureKey(master_key)
            del master_key
        except Exception:
            time.sleep(1.0)
...
class HebrewCyberApp(ctk.CTk):
    def __init__(self):
        ...
        self.log("מערכת הליבה v18.0 נטענה בהצלחה. כל מנגנוני האבטחה פעילים.", "info")
    ...
    def _worker_gen_key(self, path, pwd):
        try:
            ...
            self.after(0, lambda: self.log(f"שגיאה ביצירה: {e}", "error"))
        finally:
            # מניעת דלף זיכרון של הסיסמה
            del pwd
            gc.collect()
    ...
    def run_encryption(self):
        ...
        if self.chk_shred.get() == 1:
            msg = f"הקובץ המקורי:\n{os.path.basename(path)}\n\nיימחק לצמיתות בסיום התהליך.\n\nשים לב: בכונני SSD מודרניים, מחיקה בטוחה עשויה להיות חלקית עקב מנגנוני חומרה (Wear Leveling).\n\nהאם אתה בטוח?"
            if not self.ask_confirm("אזהרת גריסה", msg):
                return
    ...
    def _worker_encrypt(self, path):
        try:
            ...
            # הוספת גודל הקובץ המקורי למניעת התקפת חיתוך (Truncation)
            meta_dict = {
    ...
    def _worker_decrypt(self, path):
        try:
            ...
            with open(path, "rb", buffering=1024*1024) as inf:
                ...
                orig_size = meta.get("orig_size", None)
            ...
            # בדיקת שלמות נגד התקפות קטיעה (Truncation)
            if orig_size is not None and total_written != orig_size:
                raise ValueError("הקובץ נקטע או הושחת (Truncation Attack)! הגודל המשוחזר אינו תואם למקור.")
            ...
            self.after(0, lambda: self.show_error("שגיאה", f"הפענוח נכשל:\n{e}"))
        finally: self.after(0, lambda: self.set_busy(False))
